;AwaySystem 2000 Second Edition by mudpuddle - February 13, 2000
...
on *:START:{ 
  if ($version < 5.61) { asincompaterror }
    ...
    if ($asrini(awaysysver) < 2.11.999) { asimport }
    rebuildasini
...
on *:CONNECT:{ 
  if (Away isin $asrini(awaymode)) {
    asecho Setting to Online $asrini(awaymode) Mode: (4 $+ %awaymsg $+ )
    away %awaymsg
    %asconnect.status = 1
    if ($asrini(awaysystem.log) == 1) { writeaslog  $+ $colour(action) $+ *** Set to Online $asrini(awaymode) Mode (Connected to IRC server) @ $time(HH:nn:ss) $date(mm/dd/yyyy) (4 $+ %awaymsg $+  $+ $colour(action) $+ ) }
...
alias -l asversion { return AwaySystem 2000 Second Edition }
alias -l asverno { aswini awaysysver 2.11.999 }
alias -l asimport { asecho Importing existing AwaySystem settings...
  if ($asrini(awaysysver) < 2.11.999) { aswini tbarupdate 1 | aswini notify1 1
...
alias -l ascheckidle {  if ($asrini(awaymode) == Back) { if ($idle > $asrini(idleaway)) { %awaymsg = Auto IdleAway after $ascalc.dev($asrini(idleaway)) minute(s) | awaystatus.set | aswini awaymode IdleAway | if ($asrini(awaynick)) { aswini oldnick2 $asrini(awaynick) | asrmini awaynick } | setaway } } }
...
alias -l setaway { 
  ...
  if ($asrini(awaymode) == StealthAway) && ($server) { asecho Setting to StealthAway Mode (4 $+ %awaymsg $+ ) AwayPager: $group(#awaypager).status $+ , AwayLog: $group(#awaylogging).status }
...
alias -l tbar { 
  if ($asrini(awaytbar)) {
    if ($asrini(tbarupdate)) && ($asrini(awaymode) != Back) { titlebar - $iif($server,Online,Offline) $asrini(awaymode) Mode ( $+ %awaymsg $+ ) for $duration($calc($ctime - %awaytime)) - AwayPager $group(#awaypager).status $+ , AwayLog $iif($group(#awaynotices).status == on,& NoticeLog) $group(#awaylogging).status }
    if ($asrini(tbarupdate) == 0) && ($asrini(awaymode) != Back) { titlebar - $asrini(awaymode) Mode ( $+ %awaymsg $+ ) since %awayclock - AwayPager $group(#awaypager).status $+ , AwayLog $iif($group(#awaynotices).status == on,& NoticeLog) $group(#awaylogging).status }
...
alias -l asreset.confirm { 
  if ($?!="Reset ALL options to their default values?") { 
    ...
    if ($away) || (Away isin $asrini(awaymode)) { asecho Removing Away status for options reset... | setback } 
  asresetdefault | asecho All options have been reset to their default values } 
}
alias -l asresetdefault { asrmini | aswini awaymode Back | .enable #awaypager #awaylogging #awaynotices | .disable #asinputback #asidno | aswini awaysystem.log 0 | aswini awaysystemlog.log $logdir $+ awaysys.log | aswini closemsg 1 | aswini notify 1 | aswini asquietid 0 | aswini awaytimer 2700 | aswini awaytbar 1 | aswini ctcppage 1 | aswini oldidletimer 900 | aswini tbarupdate 1 | aswini notify1 1 | asverno | unset %asqaway %asnicks }
...
alias -l rebuildasini { if ($exists(awaysys.ini) == $false) || ($asrini(awaymode) == $null) { if ($away) || (Away isin $asrini(awaymode)) { setback } | asresetdefault | asecho Rebuild complete. Enter the AwaySystem Configuration to set up AwaySystem | linesep } }
alias -l asredoawaymsg {
  if ($1) {
    ...
    setaway.disp
    if ($asrini(awaymode) == StealthAway) || ($server == $null) { asecho Changed $asrini(awaymode) message (4 $+ %awaymsg $+ ) }
    tbar
    if ($asrini(awaytimer)) && ($asrini(awaymode != StealthAway)) && ($server) { .timerAway 0 $asrini(awaytimer) setaway.disp }
  ...
  else { asecho No Away message entered, Away message has not been changed }
}
alias -l logfolder {
  did -ra awaysysconf 39 $$sdir="Select Folder to store AwaySystem Logs in" $iif($asrini(awaysystemlog.log), $nofile($shortfn($asrini(awaysystemlog.log))), $logdir)
}
alias -l pagersndselect { did -ran awaysysconf 21 $$dir="Select AwayPager Sound" *.wav | did -c awaysysconf 19 | did -e awaysysconf 70 }
alias -l updaslogsize { if ($dialog(awaysysconf)) { did -a awaysysconf 73 $round($calc($file($asrini(awaysystemlog.log)).size / 1024),2) KB } } 
...
alias -l asawaynotice {
  ...
  elseif ($asrini(notify)) {
    if ($asrini(notify1)) { %asnicks = %asnicks $nick }
    .notice $nick I'm currently away ( $+ %awaymsg $+ ), your message has been logged
    if ($group(#awaypager).status == on) { .notice $nick If your message is urgent, you may page me by typing 4/ctcp $me PAGE <yourmessage> }
  }
}
#awaypager on
ctcp *:PAGE:{ 
...
#awaypager end
ctcp &*:VERSION:*:{ .ctcpreply $nick VERSION $asversion ( $+ $asrini(awaysysver) $+ ) by mudpuddle: 12http://www.geocities.com/mmircs/ }
#awaylogging on
on *:TEXT:*:?:{
...
dialog awaysysconf {
  ...
  check "&Send Away mode notify once per session", 74, 25 95 111 7, tab 100
  ...
  text "AwaySystem log file (awaysys.log) is currently:", 40, 15 95 110 7, tab 101
  text "", 73, 124 95 32 7, tab 101
...
On *:DIALOG:awaysysconf:sclick:1:{
  ...
  if ($did(63).state) && ($did(65).text) { aswini asidno $strip($did(65).text) }
...
on *:DIALOG:asawaydiag:sclick:101:{ 
  ...
  if ($did(1).text) { %awaymsg = $did(1).text }
  if ($did(1).text == $null) { %awaymsg = Away }
  if (%asqaway) { aswini awaymode StealthAway }
  if (%asqaway == $null) { aswini awaymode Away }
  if ($server == $null) { asecho Setting to Offline $asrini(awaymode) mode (4 $+ %awaymsg $+ ) - AwayPager: $group(#awaypager).status $+ , AwayLog: $group(#awaylogging).status $+ . You will be set to Online $asrini(awaymode) when you connect to IRC. }
  setaway
  unset %asqaway
}
dialog asmsgchng { 
...
menu @AwayLog,@AwayPager,@AwaySystem {
  ...
  $iif($active != @AwaySystem && $exists(awaysys.ini) && $asrini(awaymode),Clear Window &Buffer):clear
  $iif($active != @AwaySystem && $exists(awaysys.ini) && $asrini(awaymode),AwaySystem Log&Viewer):awaysystemlog
  $iif($active == @AwaySystem && $exists(awaysys.ini) && $asrini(awaymode),&Clear AwaySystem Logs...):clearawaysystemlog
  -
...
menu channel,menubar,status {
  ...
  .-
  .$iif($exists(awaysys.ini) && $asrini(awaymode),&Configure AwaySystem...):awaysys.config
  .$iif($exists(awaysys.ini) && $asrini(awaymode),AwaySystem Log&Viewer):awaysystemlog
  .-
  .&Help!
  ..$iif($exists($scriptdirashelp.msg),Help &Window, [Help file not installed]):awaysystemhelp
  ..-
  ..MmIRCS Web&site...:run http://www.geocities.com/mmircs/
  ..&Email MmIRCS...:run mailto:MmIRCS@bryansdomain.virtualave.net?subject= $+ $asversion - $asrini(awaysysver)
  ..S&ubmit a Bug...:run http://www.geocities.com/mmircs/bugsubmit.html
  ..-
  ..About AwaySyste&m:asecho $asversion (ver. $asrini(awaysysver) $+ ) by mudpuddle: February 13, 2000 - 12http://www.geocities.com/mmircs/
}
