;AwaySystem 3.01 by mudpuddle - August 18, 2000
        ...
        if ($1 == -p) {
          if ($2 == $null) {
            ...
            while (%presets <= %totalpresets) {
              echo -a 04 %presets $+ . $replace($ini(awaysys.ini,Presets,%presets),±,$chr(32)) $+ : $ascolorget($asprini(%presets))
              inc %presets
            }
            linesep
            return
...
if ($exists($shortfn(%filetemp)) == $false) { .timer 1 0 awaysyserror File does not exist! Please choose another. | return }
  ...
  if (.ini !isin %filetemp) { .timer 1 0 awaysyserror This file is not a valid .ini file! Please choose another. | return }
  ...
  %asimportchoose = %filetemp | did -a asimportwiz1 8 $left(%asimportchoose,65) $+ $iif($len(%asimportchoose) > 65,...)  | did -e asimportwiz1 2,10
}
alias -l assetnames {
...
ctcp &*:VERSION:if ($asrini(disableversionreply) != 1) { .ctcpreply $nick VERSION $asversion (August 18, 2000) by mudpuddle:12 $assite }
...
on *:DIALOG:asuwiz:sclick:2:{
  ...
  did -a $dname 6 Unloading AwaySystem Script from mIRC | did -a $dname 5 AwaySystem Uninstall Wizard - Success! | did -a $dname 6 AwaySystem Uninstall was successful. Please click Finish to close this dialog and return to mIRC | did -v $dname 9 | did -h $dname 1 | did -b $dname 2 | .unload -rs $nopath($script)
}
...
dialog asconf {
  ...
  text "&Away message:", 62, 135 70 100 7, tab 403
...
dialog asnamechk {
  ...
  size -1 -1 240 115
  option dbu
  text "This dialog allows you to remove themes from the Theme Import file.", 2, 5 5 230 7
  ...
  text "Currently installed themes:", 7, 155 32 80 7
  list 8, 155 40 80 60, vsbar
...
on *:DIALOG:asimportwiz*:init:0:{
  ...
  if ($dname == asimportwiz) { did -a $dname 22 Welcome to the AwaySystem Theme Import Wizard! $crlf $crlf This wizard will guide you through the process of importing new themes into AwaySystem 3. $crlf $crlf When you're ready to begin, click the Next button, or click Cancel to return to mIRC. }
  if ($dname == asimportwiz1) && (%asimportchoose) { did -a $dname 8 $left(%asimportchoose,65) $+ $iif($len(%asimportchoose) > 65,...) | did -ve $dname 2,10 }
  if ($dname == asimportwiz2) { did -a $dname 23 The AwaySystem Theme Import Wizard will import $ini($shortfn(%asimportchoose),0) Away Themes from: $crlf $crlf $left(%asimportchoose,85) $+ $iif($len(%asimportchoose) > 85,...) $crlf $crlf To your $mircdir $+ awaysysthemes.ini file for you to use in AwaySystem. }
...
dialog asabout {
  ...
  icon 11, 5 10 40 40
  text "", 2, 50 10 120 7
  text "Script date: August 18, 2000 ", 3, 50 20 120 7
...
on *:DIALOG:asabout:init:0:{
  ...
  did -a $dname 6 System Information as of $fulldate $crlf $crlf $+ Number of AwayNicks stored: $ini(awaysys.ini,AwayNicks,0) $crlf $+ Number of Away messages stored: $ini(awaysys.ini,Awaymsgs,0) $crlf $+ mIRC $+ $bits $version running under Windows $os $crlf $+ Computer uptime: $duration($calc($ticks / 1000)) $crlf $+ You're currently in %asawaymode mode $crlf $+ Script installed in $shortfn($scriptdir)
  did -a $dname 6 $crlf $crlf $+ ---Acknowledgements--------------------------------------------------------------- $crlf $crlf $+ Special thanks to ALGORYTHM for his beta testing, suggestions and themes for the AwaySystem Theme Pack. I would also like to thank AzBat, Joeman, Leute, and all the people who have sent me E-Mails requesting a feature being added into AwaySystem. So if you have a feature you wish to be added in a new version, please email me! $crlf $crlf $+ $&
  MmIRCS Beta Testers: ALGORYTHM, AzBat, Joeman, Leute, mudpuddle $crlf $crlf $+ Enjoy, mudpuddle <MmIRCS@bryansdomain.virtualave.net>
