;AwaySystem 4.0.112504 by MmIRCS



;/* Aliases Section, Local and Global */
...
alias awaymode {
  if (Away isin %asawaymode) { asecho You're already in Away Mode }
    ...
    if ($1 == $null) { echo  | asecho Command Syntax: /awaymode [-sp] <message/preset> | return }
    else {
...
alias -l assite { return http://www.google.com/ }
...
alias -l asreset.confirm { 
  if ($?!="Reset ALL options to their default values?") { 
    ...
    asresetdefault | .timer 1 0 awaysyserror Preferences successfully reset to defaults.
  } 
}
alias -l clearawaysystemlog { 
  if ($?!="Clear AwaySystem EventLog? $crlf $+ Note: This operation cannot be undone!") { 
    ...
    .timer 1 0 awaysyserror AwaySystem EventLog cleared.
    updaslogsize
  }
  showasprefs
}
alias -l awaysystemlog {
  if ($exists($shortfn($asrini(awaysystemlog.log)))) {   
    ...
    clear @AwaySystem
    titlebar @AwaySystem EventLog Viewer - $asrini(awaysystemlog.log) - Loading EventLog file, Please wait...
    loadbuf -p @AwaySystem $shortfn($asrini(awaysystemlog.log))
    sline -a @AwaySystem 31
    titlebar @AwaySystem EventLog Viewer - $asrini(awaysystemlog.log) - File Size: $file($asrini(awaysystemlog.log)).size bytes
  }
...
alias -l asgetcomment {
  if ($did(asconf,47).sel == 1) { var %asmsg = Classic AwaySystem theme by MmIRCS }
...
alias -l updaslogsize { if ($dialog(asconf)) { did -a asconf 306 EventLog Size: $round($calc($file($asrini(awaysystemlog.log)).size / 1024),2) $+ K } }
...
alias awaysyshlp { winhelp $shortfn($scriptdirawaysys.hlp) $asgethelptopic($1-) }
...
alias -l asgetawaymsg {
  ...
  if ($asrini(selectedtheme) > 1) { 
    ...
    var %asmessage = %ascommand $replace($astrini($calc($asrini(selectedtheme) - 1),awaymsg),@awaymsg,$+ %asawaymsg $+,@awaytime,$+ $asawayclock $+,@awaypager,$+ $awaypager $+,@awaylog,$+ $awaylogging $+))
    return %asmessage
  }
...
alias -l asgetbackmsg {
  ...
  if ($asrini(selectedtheme) > 1) {
    var %ascommand = $iif($astrini($calc($asrini(selectedtheme) - 1),cmd),ame,amsg)
    var %asmessage = %ascommand $replace($astrini($calc($asrini(selectedtheme) - 1),backmsg),@awaymsg,$+ %asawaymsg $+,@awaytime,$+ $asawayclock $+,@awayduration,$+ $awayduration $+))
    return %asmessage
  }
...
;/* Version numbers go as follows. Major.Minor.Builddate */
alias asver { return 4.0.112504 }
...
alias -l doasprefinit {
  ...
  if ($asrini(awaysystem.log) != 1) { did -b $dname 25,26 }
  if ($asrini(closemsg)) { did -c $dname 86 }
  if ($asrini(asidno)) { did -c $dname 40 | did -a $dname 41 $decode($asrini(asidno),m) }
  ...
  if (Away isin %asawaymode) { 
    ...
    if (%asawaylogging.status = off) { did -b $dname 7,8,9,86,92 }
    if (%asawaynotices.status == on) { did -c $dname 7 }
    if ($asrini(oldnick2)) { did -cb $dname 36 | did -abc $dname 37 $asrini(oldnick2) }
    else { did -abc $dname 37 $asrini(oldnick1) | did -b $dname 36,40,41,42 }
    if (%asoldnick != $null) { did -e $dname 40,42 | $iif($asrini(asidno),did -e $dname 41) }
  }
}
alias -l getdroptime {
  if ($1 == 60) { return 1 }
...
alias -l asincompaterror { asecho Error! $asversion requires mIRC 6.16 or later. You can download a new version from http://www.mirc.com/get.html | unload -rs $nopath($script) }
alias -l writeaslog {
...
alias -l asprefok {
  ...
  if ($did(40).state) && ($did(41)) { aswini asidno $encode($strip($did(41)),m) }
...
alias -l asimport {
  ...
  if ($asrini(asnickpword)) { aswini asidno $encode($asrini(asnickpword),m) | asrmini asnickpword }
}
...
alias -l updateaslogwindow {
  if ($window(@AwaySystem)) && ($asrini(awaysystemlog.log)) { 
    clear @AwaySystem
    if ($exists($shortfn($asrini(awaysystemlog.log)))) { loadbuf -p @AwaySystem " $+ $asrin(awaysystemlog.log) $+ " }
    titlebar @AwaySystem EventLog Viewer - $asrini(awaysystemlog.log) - File Size: $file($asrini(awaysystemlog.log)).size bytes
...
alias -l choosefile { var %filetemp = $$file="Pick a file to import..." themes.ini
  ...
  %asimportchoose = %filetemp | did -a asimportwiz1 8 $iif($len(%asimportchoose) > 65,...) $+ $right(%asimportchoose,65) | did -ev asimportwiz1 2,10
}
alias -l assetnames {
...
alias -l doasprefapply { asprefok | did -a $dname 304 Options saved | did -a $dname 77 Current Theme: $asgetthemename | updaslogsize }
alias -l asgetnames {
  var %tokens = 1
...
ctcp &*:VERSION:if ($asrini(disableversionreply) != 1) { .ctcpreply $nick VERSION $asversion 12 $+ $assite }
...
on *:LOAD:{ 
  if ($?!=("You loaded $asversion $crlf $crlf This script is in pre-release code Please read the below text $crlf CAREFULLY.  $crlf Clicking yes constitutes your knowledge that this code is in prerelease stage, therefore may damage data on your computer, and we are not responsible for any damages cause by the use of this script because it is in prerelease code. $crlf $crlf Do agree to these conditions?") == $false) { unload -rs $shortfn($script) }

  if ($version >= 6.16) {
    if ($asdiagver) { asecho Previous AwaySystem dialog script found, unloading... | unload -rs $shortfn($isalias(asdiagver).fname) }
    if ($isalias(asimportwiz)) { asecho Previous AwaySystem Theme Import Wizard found, unloading... | unload -rs $shortfn($isalias(asimportwiz).fname) }
    asecho $asver loaded successfully! 
    doimportthing
...
on *:CONNECT:{ 
  ...
  elseif ($asrini(idleaway)) { beginasidlechck } 
}
on *:DISCONNECT:{ 
...
on *:NICK:{
  if ($nick == $me) && ($asrini(asidno)) {
    ...
    nickserv identify %asoldnick $decode($asrini(asidno),m) | unset %asoldnick }
  }
}
on *:CLOSE:@:{
...
on *:DIALOG:asuwiz:sclick:2:{
  ...
  did -a $dname 6 Unloading AwaySystem Script from mIRC | did -a $dname 5 AwaySystem Uninstall Wizard - Success! | did -a $dname 6 AwaySystem Uninstall was successful. Please click Finish to close this dialog and return to mIRC. $+ $crlf $crlf $+ Thank you for using AwaySystem $asver by MmIRCS. $assite  | did -v $dname 9 | did -h $dname 1 | did -b $dname 2 | .unload -rs $nopath($script)
}
on *:DIALOG:asuwiz:sclick:10:did -c $dname 10
on *:DIALOG:asuwiz:sclick:20:did -bc $dname 10,11,12,13,14,15
on *:DIALOG:asuwiz:sclick:21:did -eu $dname 11,12,13,14,15 | did -e $dname 10
on *:Dialog:asconf:init:0:{
  ...
  if ($asrini(prefhidestatus))  { did -h $dname 302,303,304,305,306 | did -u $dname 208 | dialog -sb $dname -1 -1 273 159 }
  if ($asrini(prefhidestatus) != 1) { did -v $dname 302,303,304,305,306 | did -c $dname 208 | dialog -sb $dname -1 -1 273 171 }
  doasprefinit
}
...
on *:Dialog:asconf:sclick:22:{
  if ($did(22).state) { did -e $dname 25,26 }
  else { did -b $dname 25,26 }
}
...
on *:Dialog:asconf:sclick:36:{
  if ($did(36).state) { did -e $dname 37,40,42 | $iif($did(40).state,did -e $dname 41) }
  if ($did(36).state == 0) { did -b $dname 37,40,41,42 }
}
...
on *:DIALOG:asconf:sclick:61:{ if ($did(59,$did(59).sel) != $null) { asprmini $did(59).sel | remini awaysys.ini PresetNicks $replace($did(83),$chr(32),±) | did -rn $dname 83,63,96 | did -d $dname 59 $did(59).sel } }
...
on *:Dialog:asconf:menu:201:doasprefapply
on *:Dialog:asconf:menu:202:asprefok | dialog -x $dname
on *:Dialog:asconf:menu:203:dialog -x $dname
on *:Dialog:asconf:menu:207:awaysystemlog
on *:Dialog:asconf:menu:208:{
  if ($asrini(prefhidestatus)) { did -v $dname 302,303,304,305,306 | did -c $dname 208 | dialog -sb $dname -1 -1 273 171 | aswini prefhidestatus 0 }
  else { did -h $dname 302,303,304,305,306 | did -u $dname 208 | dialog -sb $dname -1 -1 273 159 | aswini prefhidestatus 1 }
}
...
;run $assite
on *:DIALOG:asconf:menu:213:.timer 1 0 awaysyserror Feature not yet updated.
;run mailto:email@mail.com?subject= $+ $asversion
on *:DIALOG:asconf:menu:214:.timer 1 0 awaysyserror Feature not yet updated.
;run $assite $+ bugsubmit.html
on *:DIALOG:asconf:menu:215:.timer 1 0 awaysyserror Feature not yet updated.
...
on *:DIALOG:asconf:menu:231:{ 
  if ($exists($mircdir $+ awaysysthemes.ini)) {
    var %exportfile = $sfile(C:,Export AwaySystem Themes,Save)
    if (%exportfile) { var %exportfile = %exportfile $+ .ini
      if ($exists(" $+ %exportfile $+ ") == $true) { 
        if ($?!="File already exists. Overwrite?") { var %asexportswitch = -o } 
        else { halt }
      }
    .copy %asexportswitch " $+ $mircdir $+ awaysysthemes.ini" " $+ %exportfile $+ " | asecho Themes exported to %exportfile }
  }
  else { .timer 1 0 awaysyserror No themes file exists! }
}
...
on *:DIALOG:asconf:sclick:502:doasprefapply
...
on *:DIALOG:asabout:init:0:{
  ...
  did -a $dname 3 $chr(169) Copyright 2004-2005 MmIRCS
  did -a $dname 6 System Information as of $fulldate $crlf $crlf $+ AwayNicks saved: $ini(awaysys.ini,AwayNicks,0) $crlf $+ Away messages saved: $ini(awaysys.ini,Awaymsgs,0) $crlf $+ mIRC $+ $bits $version running under Windows $os $crlf $+ Computer uptime: $duration($calc($ticks / 1000)) $crlf $+ You're currently in $iif(%asawaymode != Back && %asawaymode != Away,a damaged config,%asawaymode) mode $iif(%asawaymode != Back && %asawaymode != Away,- Please repair your config file. See help for more details.) $crlf $+ Script installed in $scriptdir
  did -a $dname 6 $crlf $crlf $+ ---Acknowledgements---------------------- $crlf $crlf $+ Thanks to all MmIRCS supporters over the years for encouraging, and contributing to the AwaySystem project. AwaySystem has been an ongoing project since September 1999, and has undergone numerous changes to functionality and costmetics. All thanks to emails for suggestions for features. $crlf $+ Again, thank you for your contributions, and enjoy this version of AwaySystem for mIRC from MmIRCS.
...
dialog asuwiz {
  ...
  check "Unload AwaySystem script file", 10, 15 60 82 7 
...
dialog asconf {
  ...
  option dbu
  menu "&File", 200
  item "&Save Changes", 201
  item "Exit && Save &Changes", 202
  item "E&xit without Saving Changes", 203 
  menu "&Tools", 206
  item "Show &Statusbar", 208
  item break, 300
  menu "AwaySystem &EventLog Utilities" 219, 206
  item "AwaySystem EventLog &Viewer", 207
  item "&Clear AwaySystem EventLog...", 24
  menu "AwaySystem Themes Utilities" 230, 206
  item "&AwaySystem Theme Import Wizard...", 220
  item "E&xport AwaySystem Themes...", 231
  item break, 304, 206
  item "&Clear AwayNicks && Away Messages...", 218
  item "&Revert ALL Settings to Defaults...", 209
  ...
  check "&Allow Pages in Back Mode", 3, 22 40 74 7, tab 400
  check "Send &Reply on message reception", 4, 22 50 90 7, tab 400
  box "AwayLog Options", 5, 10 60 125 63, tab 400
  check "Enable &AwayLog", 6, 15 70 50 7, tab 400
  button "Triggers...", 92, 75 68 35 12, tab 400
  check "Include &Notices", 7, 22 80 48 7, tab 400
  check "Close /ms&g Windows", 86, 75 85 59 7, tab 400
  check "Include Actio&ns", 8, 22 90 50 7, tab 400
  check "&Let user know you're in Away Mode", 9, 22 100 110 7, tab 400
  check "Send Notice to User only &One Time", 10, 29 111 93 7, tab 400
  ...
  check "&Save old Titlebar and replace on Back Mode", 17, 147 53 115 7, tab 400
  text "Activate Auto &IdleAway:", 18, 139 65 59 7, tab 400
  combo 19, 198 64 63 100, drop tab 400
  check "Set to Back Mode on &Channel Input", 20, 139 77 118 7, tab 400
  box "AwaySystem EventLog Options", 21, 134 87 130 36, tab 400
  ...
  button "Choose &Folder...", 26, 216 108 45 13, tab 400
  box "Sound Events", 27, 10 20 254 103, tab 401
  text "Choose sound to play for the event:", 28, 15 30 100 7, tab 401
  text "AwayPager >>", 29, 15 43 38 7, tab 401
  button "No Sound", 30, 53 41 160 12, tab 401
  button "Preview", 31, 220 41 35 12, tab 401
  text "AwayLog >>", 32, 15 63 35 7, tab 401
  button "No Sound", 33, 53 61 160 12, tab 401
  button "Preview", 34, 220 61 35 12, tab 401
  ...
  text "when I set to Away Mode", 38, 182 90 70 7, tab 401
  ...
  list 59, 15 55 70 70, vsbar tab 403
  button "Ne&w", 78, 90 60 35 12, tab 403
  ...
  text "Use this file for random messages:", 68, 15 56 85 7, tab 404
  ...
  check "&Tell user you're away when they join", 93, 160 110 100 7, tab 404
  icon 503, 5 134 12 12
  text "AwaySystem by MmIRCS", 504, 19 136 99 7
...
dialog asawaydiag { 
  ...
  size -1 -1 177 52 
  option dbu
  text "&Away message:", 202, 5 7 38 7
  combo 1, 44 5 118 80, drop edit
  text "0", 4, 164 7 13 7, center
  check "Away&Pager on", 2, 5 21 44 7 
  check "Away&Log on", 3, 58 21 39 7, 3state
  button "OK", 101, 5 35 27 12, OK default
  button "Cancel", 102, 35 35 27 12, cancel
  check "Away&Nick:", 103, 65 38 33 7
  combo 105, 100 36 73 40, drop edit
  text "<-(", 106, 100 21 9 7
  check "= No Notice Logging)", 104, 109 21 60 7, 3state
} 
dialog asmsgchng { 
  title "Change Away Message" 
  size -1 -1 177 42 
  option dbu
  text "&New message:", 200, 5 7 37 7
...
dialog asabout {
  ...
  text "", 2, 50 10 120 7
  text "", 3, 50 20 120 7
  text "All Rights Reserved", 4, 50 30 120 7
...
menu @AwayLog,@AwayPager,@AwaySystem {
  $iif($exists(awaysys.ini) == $false || %asawaymode != Away && %asawaymode != Back && %asawaymode != StealthAway,Repair config file):asresetdefault | .timer 1 0 awaysyserror Successfully repaired configuration settings.
  ...
  $iif($active != @AwaySystem && $ascheckini,AwaySystem EventLog &Viewer):awaysystemlog
  $iif($active == @AwaySystem && $ascheckini,&Clear AwaySystem EventLog...):clearawaysystemlog
  -
  $iif($ascheckini,AwaySystem &Preferences...):awaysys.config
  Abo&ut AwaySystem:awaysysabout
  &Help!:awaysyshlp
}
menu channel,menubar,status {
  &AwaySystem ( $+ $iif(%asawaymode != Back && %asawaymode != Away && %asawaymode != StealthAway && %asawaymode != IdleAway,Unknown,%asawaymode) Mode)
  ..$iif($exists(awaysys.ini) == $false || %asawaymode != Away && %asawaymode != Back && %asawaymode != StealthAway && %asawaymode != IdleAway,Repair config file):asresetdefault | .timer 1 0 awaysyserror Successfully repaired configuration settings.
  ..-
  ...
  ..$iif($ascheckini && %asawaymode == Away || %asawaymode == StealthAway || %asawaymode == IdleAway,Set to &Back Mode):assetback
  ..$iif($ascheckini && %asawaymode == Away || %asawaymode == StealthAway || %asawaymode == IdleAway,Change Away &Message...):$dialog(asmsgchng,asmsgchng)
  ..-
  ..$iif($ascheckini && %asawaymode == Away || %asawaymode == StealthAway || %asawaymode == Back || %asawaymode == IdleAway,AwaySystem &Preferences...):awaysys.config
  ..$iif($ascheckini && %asawaymode == Away || %asawaymode == StealthAway || %asawaymode == Back || %asawaymode == IdleAway,AwaySystem EventLog &Viewer):awaysystemlog
  ..-
  ..Abo&ut AwaySystem...:awaysysabout
  ..&Help!:awaysyshlp
}
